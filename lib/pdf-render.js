...
async function renderHtmlToPdf(html, pdfOptions = {}) {
  ...
  try {
    ...
    // Puppeteer 22+ changed page.pdf() to return a Uint8Array instead of a
    // Buffer. Express's res.send checks Buffer.isBuffer() to choose the binary
    // path; a plain Uint8Array returns false and falls into res.json(), which
    // JSON-encodes the bytes and breaks the download. Wrap in Buffer.from()
    // so every caller of this helper gets a real Buffer regardless of which
    // Puppeteer major version is installed.
