...
// ─── 3-Week Winter Challenge (paid beta) ───────────────────────────────────
// One-time $299 to join the challenge; fully refunded if the student
// completes it (handled manually in Stripe by Julia — the platform just
// tracks who qualifies).
const CHALLENGE_PRICE_CENTS = 29900;

function getChallengePriceCents() { return CHALLENGE_PRICE_CENTS; }

async function createChallengePaymentIntent(userId, userEmail) {
  if (!stripe) throw new Error('Stripe not configured');
  return stripe.paymentIntents.create({
    amount:   CHALLENGE_PRICE_CENTS,
    currency: CURRENCY,
    metadata: {
      app:     'creatives-garden',
      purpose: 'winter-challenge',
      user_id: String(userId),
    },
    receipt_email: userEmail || undefined,
    automatic_payment_methods: { enabled: true },
...
// Fetch a PaymentIntent to confirm its status server-side after the client
// reports success — so we grant access immediately instead of waiting on the
// webhook (which stays as an idempotent backstop).
