...
async function runDailyReminders({ now, dryRun = false, log } = {}) {
  ...
  for (const u of users) {
    ...
    // Already logged a recording for their local today? Then the nudge has
    // nothing to nudge — skip it rather than nag someone who's already done
    // the thing. Checked per local date, so it matches what they see on the
    // dashboard. No milestone is claimed; if they somehow delete the entry
    // before their hour passes, a later run this same day can still remind.
    if (db.hasCuttingForDate(u.id, dateKey)) {
      _log(`  · ${u.name} (${u.email}) — already logged a recording for ${dateKey} → skip`);
      continue;
    }
